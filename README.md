...
- 接雨水（trap）hard
  ...
  - 动态规划版本，如果知道每个元素左右最大的值则不需要额外的遍历，通过从前往后算出每个元素左边最大值，再从右往左算出每个元素右边最大值，再计算每日接到的雨水就简单的多。并且时间复杂度降为了 O(n)。
...
1. 确定 dp[i]数组及下标含义(也可能是个二维数组 dp[i][j])
2. 确定 dp[i]的递推公式。（需要找规律）
3. dp 数组初始化
4. 确定遍历顺序
5. 举例推导 dp 数组
